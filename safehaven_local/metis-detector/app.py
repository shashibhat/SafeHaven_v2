...
class Config:
    ...
    model_task = os.getenv("MODEL_TASK", "detect").lower()
...
def _classify(image: np.ndarray) -> List[List[float]]:
    model = _get_model()
    ...
    # SafeHaven core only needs class_id + score, so full-frame bbox is sufficient.
    return [[class_id, score, 0.0, 0.0, 1.0, 1.0]]


def _detect(image: np.ndarray) -> List[List[float]]:
    model = _get_model()
    results = model.predict(image, verbose=False)
    if not results:
        return []
...
def on_startup():
    _setup_logging()
    LOGGER.info(
        "metis-detector startup mock=%s model_task=%s model_dir=%s",
        Config.mock,
        Config.model_task,
        Config.model_dir,
    )
...
async def detect(request: Request):
    ...
    if Config.model_task in ("classify", "classification", "cls"):
